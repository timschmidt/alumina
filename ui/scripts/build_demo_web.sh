...
if [[ "${OPTIMIZE}" = true ]]; then
  ...
  wasm-opt "${FINAL_WASM_PATH}" -Oz --fast-math -o "${FINAL_WASM_PATH}" # add -g to get debug symbols
fi
...
brotli --best --keep --force "${FINAL_WASM_PATH}"
brotli --best --keep --force docs/${OUT_FILE_NAME}.js